...
// --------------------------------------
// IAS 12 (Phase 0 + Phase 1)
// - Authorities
// - Rate sets (+ effective-dated lines)
// - Org settings (incl. accounts for later deferred tax posting)
// --------------------------------------
...
async function resolveRateForPeriodEnd({ orgId, periodId, rateSetId }) {
  ...
  const resolved = await resolveRateLineForPeriodEnd({ rateSetId, periodEndDate: period.end_date });
  return { period_end_date: period.end_date, ...resolved };
}
...
const { rows: overlaps } = await pool.query(
  ...
  WHERE rate_set_id = $1
    AND (
      (effective_to IS NULL OR effective_to >= $2::timestamp)
      AND ($3::timestamp IS NULL OR effective_from <= $3::timestamp)
    )
  LIMIT 1
  `,
  [rateSetId, effectiveFrom, effectiveTo]
