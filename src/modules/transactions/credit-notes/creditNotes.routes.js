...
router.put("/:id", idempotency({ required: true }), requirePermission("transactions.credit_note.manage"), async (req, res, next) => {
  try {
    const orgId = req.user.organization_id;
    const actorUserId = req.user.id;
    const payload = validate(createCreditNoteSchema, req.body);
    res.json(await svc.updateDraftCreditNote({ orgId, actorUserId, id: req.params.id, payload }));
  } catch (e) { next(e); }
});
