...
// Bulk export (CSV)
router.get("/export", requirePermission("hr.grades.export"), async (req, res, next) => {
  try {
    const orgId = req.user.organization_id; 
    const csv = await svc.exportGradesCsv({ orgId, query: req.query }); 
...
router.post("/import", idempotency({ required: true }), requirePermission("hr.grades.import"), async (req, res, next) => {
  try {
    const orgId = req.user.organization_id; 
    const actorUserId = req.user.id; 
    const { grades, mode } = req.body || {}; 
    res.status(200).json(await svc.importGrades({ orgId, actorUserId, grades, mode, audit: req.audit, writeAudit })); 
  } catch (e) { next(e);  }
}); 
...
router.post(
  ...
  requirePermission("hr.grades.import_csv"),
  ...
  async (req, res, next) => {
    try {
      ...
      res.status(200).json(await svc.importGradesCsv({ orgId, actorUserId, csvText, mode, audit: req.audit, writeAudit })); 
    } catch (e) { next(e);  }
  }
); 
