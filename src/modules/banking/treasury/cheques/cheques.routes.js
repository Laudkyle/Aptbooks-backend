...
router.post('/:chequeId/clear', idempotency({ required: true }), requirePermission('banking.treasury.execute'), async (req, res, next) => { try { res.json(await svc.clear(req.user.organization_id, req.params.chequeId, req.user.id, req.body || {})); } catch (e) { next(e); } });
router.post('/:chequeId/void', idempotency({ required: true }), requirePermission('banking.treasury.manage'), async (req, res, next) => { try { res.json(await svc.voidCheque(req.user.organization_id, req.params.chequeId, req.user.id, req.body || {})); } catch (e) { next(e); } });
router.post('/:chequeId/bounce', idempotency({ required: true }), requirePermission('banking.treasury.manage'), async (req, res, next) => { try { res.json(await svc.bounce(req.user.organization_id, req.params.chequeId, req.user.id, req.body || {})); } catch (e) { next(e); } });
