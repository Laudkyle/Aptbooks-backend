...
async function create(orgId, actorUserId, payload) {
  ...
  return repo.create(orgId, { batchNo: payload.batchNo || genCode('PAB'), name: payload.name, scheduledDate: payload.scheduledDate || null, notes: payload.notes || null }, actorUserId);
}
...
async function addItems(orgId, batchId, payload) {
  ...
  return withTransaction(async (client) => {
    ...
    for (const item of items) {
      if (!['payment_run','bank_transfer'].includes(item.itemType)) throw new AppError(400, 'itemType must be payment_run or bank_transfer');
      if (!item.itemId) throw new AppError(400, 'itemId is required');
      if (item.itemType === 'payment_run') {
        const row = await paymentRunsRepo.get(orgId, item.itemId, client);
        if (!row) throw new AppError(404, `Payment run not found: ${item.itemId}`);
        ...
        await paymentRunsRepo.replaceStatus(orgId, item.itemId, row.status === 'draft' ? 'submitted' : row.status, { approvalBatchId: batchId }, client);
      } else {
        const row = await bankTransfersRepo.get(orgId, item.itemId, client);
        if (!row) throw new AppError(404, `Bank transfer not found: ${item.itemId}`);
        ...
        await bankTransfersRepo.updateStatus(orgId, item.itemId, row.status === 'draft' ? 'submitted' : row.status, { approvalBatchId: batchId }, client);
      }
      normalized.push({ itemType: item.itemType, itemId: item.itemId });
    }
