...
router.post('/:batchId/submit', idempotency({ required: true }), requirePermission('banking.treasury.manage'), async (req, res, next) => { try { res.json(await svc.submit(req.user.organization_id, req.params.batchId, req.user.id)); } catch (e) { next(e); } });
...
router.post('/:batchId/cancel', idempotency({ required: true }), requirePermission('banking.treasury.manage'), async (req, res, next) => { try { res.json(await svc.cancel(req.user.organization_id, req.params.batchId, req.body?.reason, req.user.id)); } catch (e) { next(e); } });
