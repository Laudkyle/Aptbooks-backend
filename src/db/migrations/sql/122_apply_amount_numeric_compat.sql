...
-- These open-items views depend on amount_applied columns in allocation/application tables.
-- Drop them first so legacy databases can change the underlying column types safely.
...
-- Recreate latest open-items views after the type changes.
