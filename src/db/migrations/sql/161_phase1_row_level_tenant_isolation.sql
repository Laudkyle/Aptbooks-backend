-- Phase 1 production hardening: fail-closed tenant isolation.
--
-- Runtime contract:
--   SELECT set_config('app.current_organization_id', '<uuid>', false)
-- is applied by src/db/pool.js for authenticated tenant work and scrubbed before
-- a pooled connection is reused. The production runtime role MUST NOT own these
-- tables and MUST NOT have BYPASSRLS (see db/admin/production_roles.sql).
--
-- Authentication/bootstrap/control-plane tables are intentionally excluded
-- because they are required to establish tenant context or are global runtime
-- infrastructure. Financial/business tables are protected automatically.

CREATE OR REPLACE FUNCTION aptbooks_current_organization_id()
RETURNS uuid
LANGUAGE sql
STABLE
AS $$
...
COMMENT ON FUNCTION aptbooks_current_organization_id() IS
  'Returns the tenant organization id bound to the current database session by the AptBooks runtime pool.';
