-- 077_stage3_open_items_include_discounts.sql
BEGIN;
...
-- ------------------------------------------------------------
-- AR open items (invoice total - posted receipt allocations - issued credit note applications)
-- ------------------------------------------------------------
CREATE VIEW reporting_ar_open_items AS
WITH ralloc AS (
  SELECT
    cra.invoice_id,
    SUM(cra.amount_applied + COALESCE(cra.discount_taken,0)) AS allocated
  ...
  SELECT
    cna.invoice_id,
    SUM(cna.amount_applied) AS applied
...
SELECT
  ...
  (i.total - COALESCE(r.allocated,0) - COALESCE(cn.applied,0)) AS outstanding
FROM invoices i
LEFT JOIN ralloc r ON r.invoice_id = i.id
LEFT JOIN cnalloc cn ON cn.invoice_id = i.id;

-- ------------------------------------------------------------
-- AP open items (bill total - posted payment allocations - issued debit note applications)
-- ------------------------------------------------------------
CREATE VIEW reporting_ap_open_items AS
WITH palloc AS (
  SELECT
    vpa.bill_id,
    SUM(vpa.amount_applied + COALESCE(vpa.discount_taken,0)) AS allocated
  ...
  SELECT
    dna.bill_id,
    SUM(dna.amount_applied) AS applied
...
SELECT
  ...
  (b.total - COALESCE(p.allocated,0) - COALESCE(dn.applied,0)) AS outstanding
FROM bills b
