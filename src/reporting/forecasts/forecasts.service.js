...
async function submitVersion({ orgId, forecastId, versionId, actorUserId, req }) {
  ...
  const updated = await withTransaction(async (client) => {
    const { version, forecast, lines } = await getForecastVersionSnapshot({ orgId, forecastId, versionId, client });
    ...
    await documentableSvc.submitEntityForApproval({
      orgId, actorUserId, entityType: "forecast", entity: version, workflowDocumentId: version.workflow_document_id,
      snapshot: { header: version, lines, related: { forecast }, meta: { workflow_status: version.workflow_status, status: version.status } },
      client,
      persistWorkflowDocumentId: async (workflowDocumentId) => {
        await client.query(`UPDATE forecast_versions SET workflow_document_id=$4, updated_at=NOW() WHERE organization_id=$1 AND forecast_id=$2 AND id=$3`, [orgId, forecastId, versionId, workflowDocumentId]);
      }
    });
    return repo.updateVersionWorkflow({ orgId, forecastId, versionId, patch: { workflowStatus: "in_review", submittedAt: new Date().toISOString(), submittedByUserId: actorUserId, rejectionReason: null } });
  });
  await writeAudit({ organizationId: orgId, actorUserId, action: "reporting.forecast.version.submit", entityType: "forecast_version", entityId: versionId, ip: req.ip, userAgent: req.headers["user-agent"], before: null, after: updated });
  return updated;
}
...
async function approveVersion({ orgId, forecastId, versionId, actorUserId, req }) {
  const updated = await withTransaction(async (client) => {
    ...
    await documentableSvc.approveEntityDocument({ orgId, actorUserId, entityType: "forecast", workflowDocumentId: version.workflow_document_id, creatorUserId: version.created_by_user_id, client });
    return repo.updateVersionWorkflow({ orgId, forecastId, versionId, patch: { workflowStatus: "approved", approvedAt: new Date().toISOString(), approvedByUserId: actorUserId } });
  });
  await writeAudit({ organizationId: orgId, actorUserId, action: "reporting.forecast.version.approve", entityType: "forecast_version", entityId: versionId, ip: req.ip, userAgent: req.headers["user-agent"], before: null, after: updated });
  return updated;
}
...
async function rejectVersion({ orgId, forecastId, versionId, reason, actorUserId, req }) {
  const updated = await withTransaction(async (client) => {
    ...
    await documentableSvc.rejectEntityDocument({ orgId, actorUserId, entityType: "forecast", workflowDocumentId: version.workflow_document_id, creatorUserId: version.created_by_user_id, comment: reason || "Rejected", client });
    return repo.updateVersionWorkflow({ orgId, forecastId, versionId, patch: { workflowStatus: "rejected", rejectedAt: new Date().toISOString(), rejectedByUserId: actorUserId, rejectionReason: reason || "Rejected" } });
  });
  await writeAudit({ organizationId: orgId, actorUserId, action: "reporting.forecast.version.reject", entityType: "forecast_version", entityId: versionId, ip: req.ip, userAgent: req.headers["user-agent"], before: null, after: updated });
  return updated;
}
