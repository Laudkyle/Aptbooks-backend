...
router.post("/types", idempotency({ required: true }), requirePermission("documents.manage"), async (req, res, next) => {
  try {
    ...
    if (e?.code === "23505") return next(new AppError(409, "Document type already exists")); 
    next(e); 
  }
}); 

router.get("/types", requirePermission("documents.read"), async (req, res, next) => {
  try {
    const orgId = req.user.organization_id; 
...
router.post("/approval-levels", idempotency({ required: true }), requirePermission("documents.manage"), async (req, res, next) => {
  try {
    ...
    if (e?.code === "23505") return next(new AppError(409, "Approval level code/sequence already exists")); 
    next(e); 
  }
}); 

router.get("/approval-levels", requirePermission("documents.read"), async (req, res, next) => {
  try {
    const orgId = req.user.organization_id; 
...
router.post("/", idempotency({ required: true }), requirePermission("documents.create"), async (req, res, next) => {
  try {
    ...
    const created = await svc.createDocument({ orgId, userId: req.user.id, payload }); 
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!originalFilename) throw new AppError(400, "Missing x-filename header"); 
      if (!Buffer.isBuffer(req.body) || req.body.length === 0) throw new AppError(400, "Empty upload body"); 
      ...
      await writeAudit({
        ...
        ip: req.audit?.ip,
        userAgent: req.audit?.userAgent,
        after: { version_no: result.version.version_no, version_id: result.version.id }
      }); 
...
// Download a version
router.get("/:id/versions/:versionId/download", requirePermission("documents.read"), async (req, res, next) => {
  try {
    const orgId = req.user.organization_id; 
...
router.post("/:id/submit", idempotency({ required: true }), requirePermission("documents.create"), async (req, res, next) => {
  try {
    ...
    const result = await svc.submitDocument({ orgId, documentId: req.params.id }); 
...
router.post("/:id/approve", idempotency({ required: true }), requirePermission("approvals.act"), async (req, res, next) => {
  try {
    ...
    await writeAudit({
      ...
      ip: req.audit?.ip,
      userAgent: req.audit?.userAgent,
      after: { approval_id: result.approval?.id, document_state: result.document?.workflow_state_code }
    }); 
